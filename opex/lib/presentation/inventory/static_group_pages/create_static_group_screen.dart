...
class CreateStaticGroupScreen extends StatefulWidget {
  final bool? edit;
  const CreateStaticGroupScreen({Key? key, this.edit}) : super(key: key);

  @override
  State<CreateStaticGroupScreen> createState() => _CreateStaticGroupScreenState();
}

class _CreateStaticGroupScreenState extends State<CreateStaticGroupScreen> {
  bool isActive = true;
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocListener(
      listeners: [
        ...
        BlocListener<InventoryBloc, InventoryState>(
          listener: (context, state) {
            if (state is CreateStaticGroupLoading) {
              showSnackBar(context,
                  message: "Static Group Creation Loading",
                  color: ColorPalette.green);
            } else if (state is CreateStaticGroupSuccess) {
              showSnackBar(context,
                  message: state.successMessage??"",
                  color: ColorPalette.green);
              context.read<InventoryBloc>().add(const GetStaticGroupListEvent('','',''));
              Navigator.pop(context);
            } else if (state is CreateStaticGroupFailed) {
              showSnackBar(context,
        ...
        BlocListener<InventoryBloc, InventoryState>(
          listener: (context, state) {
            if (state is StaticGroupReadLoading) {

            } else if (state is StaticGroupReadSuccess) {
            ...
            } else if (state is StaticGroupReadFailed) {
              showSnackBar(context,
                  message: state.error, color: ColorPalette.black);
            }
          },
        ...
        BlocListener<InventoryBloc, InventoryState>(
          listener: (context, state) {
            if (state is StaticGroupUpdateLoading) {
              showSnackBar(context,
                  message: "Static Group Updation Loading",
                  color: ColorPalette.green);
            } else if (state is StaticGroupUpdateSuccess) {
              showSnackBar(context,
                  message: state.successMessage??"",
                  color: ColorPalette.green);
              context.read<InventoryBloc>().add(const GetStaticGroupListEvent('','',''));
      ...
      child: Scaffold(
        ...
        appBar: PreferredSize(
          ...
          child: BackAppBar(
            label: widget.edit==true?"Update Static Group":"Create Static Group", isAction: false,
                ...
                children: [
                  ...
                  widget.edit==true?GradientButton(
                      onPressed: () {
                        ...
                        context.read<InventoryBloc>().add(UpdateStaticGroupEvent(
                            id: modelData?.id??0,
                            isActive: isActive,
                            image: imageId,
                      ...
                      onPressed: () {
                        ...
                        context.read<InventoryBloc>().add(StaticGroupCreateEvent(
                            image: imageId,
                            name: nameController.text,
