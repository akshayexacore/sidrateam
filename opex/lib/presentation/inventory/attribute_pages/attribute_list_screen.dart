...
class AttributeListScreen extends StatefulWidget {
  const AttributeListScreen({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<InventoryBloc, InventoryState>(
      listener: (context, state) {
        if (state is DeleteAttributeSuccess) {
          showSnackBar(context,
              message: "Attribute Deleted Successfully",
              color: ColorPalette.green);
          context
              .read<InventoryBloc>()
              .add(const GetAttributeListEvent('', '', ''));
            ...
            child: SingleChildScrollView(
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    SearchCard(
                      hint: "Search Attributes...!",
                      onchange: (jj) {
                        context
                            .read<InventoryBloc>()
                            .add(GetAttributeListEvent(jj ?? "", '', ''));
                        setState(() {});
                      },
                    ),
                    SizedBox(
                    ...
                    BlocBuilder<InventoryBloc, InventoryState>(
                      builder: (context, state) {
                        if (state is GetAttributeListLoading) {
                          return Container(
                              height: 200,
                              width: w,
                                                              ...
                                                              GradientButton(
                                                                  ...
                                                                  onPressed:
                                                                    ...
                                                                    context
                                                                        .read<
                                                                            InventoryBloc>()
                                                                        .add(DeleteAttributeEvent(division[index].id ??
                                                                            0));
                                                                    Navigator.pop(
                                                                        context);
                                                                  },
                        ...
                        if (state is GetAttributeListFailed) {
                          // group.clear();
                          division.clear();
                          // setState(() {});
