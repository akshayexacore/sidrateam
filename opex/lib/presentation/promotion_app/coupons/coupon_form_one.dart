...
import 'coupon_bloc/coupon_bloc.dart';
import 'coupons_form.dart';


class CouponFormOne extends StatefulWidget {
  String type;
  final String? restorationId;
  final bool? editCopen;
  CouponFormOne({Key? key, this.type = "",this.restorationId,  this.editCopen}) : super(key: key);

  @override
  State<CouponFormOne> createState() => _NewPromotionState();
}

class _NewPromotionState extends State<CouponFormOne> {
...
//date
  ...
  void initState() {
    ...
    context.read<CouponBloc>().add(GetOfferApplyingBasedOnReadCouponEvent());
        ...
        BlocListener<CouponBloc, CouponState>(
          listener: (context, state) {
            ...
            if (state is GetOfferAppliyingBasedOnReadCouponSuccess) {
              basedOn = state.discount.couponBasedOn!;
              setState(() {
        ...
        BlocListener<CouponBloc, CouponState>(
          listener: (context, state) {
            if (state is GetCouponReadLoading) {
              ...
              readCoupen = state.couponList;
              print("read$readCoupen");
              print("segmentListRead$segmentListttt");
              promotionTitle.text = readCoupen?.name ?? "";
              promotionDescription.text = readCoupen?.description ?? "";
              basedOnValue = readCoupen?.basedOn ?? "";
              offerperiodId = readCoupen?.offerPeriodId ?? 0;
              isAvailable = readCoupen?.isAvailableToAll ??false;
              discountSegment = readCoupen?.segments ?? [];
              imageId = readCoupen?.image ?? "";
              imageFileName=readCoupen?.image??"";
              drop = readCoupen?.channelCode ?? "";
              displayName.text=readCoupen?.displayName??"";
              promotiondiscount.text=readCoupen?.priceOrPercentage.toString()??"";
              offerPeriodName=readCoupen?.offerPerodName??"";
              availableCustomerList=readCoupen?.customer??[];
              selectedCustomerList=readCoupen?.customer??[];
              for (var i = 0; i < readCoupen!.segments!.length; i++) {
                segmentListttt.add(
                    readCoupen?.segments?[i].segmentName ?? "");
              }
              for(int i=0;i<segmentListttt.length;i++){
                lifeStyle="$lifeStyle${segmentListttt[i]},";
              }
              for(int i=0;i<readCoupen!.customer!.length;i++){
                customercode.add(readCoupen?.customer?[i].customerGroupCode??"");
              }
              print("segmentListRead$segmentListttt");
              print("segmentListRead${readCoupen?.isAvailableToAll}");
              setState(() {
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                widget.editCopen==true?ReadDropDownCard(
                  selValue: drop,
                ...
                GradientButton(
                    ...
                    onPressed: () {
                      ...
                      PersistentNavBarNavigator.pushNewScreen(
                        context,
                        screen: CouponsForm(
                            editCoupen: widget.editCopen??false,
                          changeedSeg: changeSegment,
                          segmentList: segmentListttt,
                          basedOn: basedOnValue,
                          coupenList: readCoupen,
                          promotionDescription: promotionDescription.text,
                          promotionName: promotionTitle.text,
                          displayName: displayName.text,
                          discountPercentagePrice: double.tryParse(promotiondiscount.text),
