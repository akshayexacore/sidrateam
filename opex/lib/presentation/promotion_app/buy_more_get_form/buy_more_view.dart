...
class BuyMoreView extends StatefulWidget {
  const BuyMoreView({Key? key}) : super(key: key);

  @override
  State<BuyMoreView> createState() => _BuyMoreViewState();
}

class _BuyMoreViewState extends State<BuyMoreView> {
  BuyMoreList? readBuyMore;
  String lifeStyle="";
  List<String> segment=[];
  @override
  Widget build(BuildContext context) {
    ...
    return MultiBlocListener(
      listeners: [
        BlocListener<BuyMoreBloc, BuyMoreState>(
          listener: (context, state) {
            if(state is GetBuyMoreReadLoading){
              ...
              readBuyMore=state.buyMoreList;
              for(var i=0;i<readBuyMore!.segments!.length;i++){
                segment.add(readBuyMore?.segments?[i].segmentName??"");
              }
              for(int i=0;i<segment.length;i++){
                lifeStyle="$lifeStyle${segment[i]},";
              }
              print("lifee$lifeStyle");
              print("lifee${readBuyMore?.segments?.length}");
              setState(() {
        ...
        BlocListener<BuyMoreBloc, BuyMoreState>(
          listener: (context, state) {
            if (state is DeleteBuyMoreLoading) {
              showSnackBar(context,
                  message: "Loading",
                  color: ColorPalette.green);
            } else if (state is DeleteBuyMoreSuccess) {
              showSnackBar(context,
                  message: "Buy More Deleted Successfully",
                  color: ColorPalette.green);
              context.read<BuyMoreBloc>().add( PaginatedBuyMoreListEvent("",""));
              Navigator.pop(context);

            } else if (state is DeleteBuyMoreFailed) {
              showSnackBar(context,
      ...
      child: Scaffold(
        ...
        appBar: PreferredSize(
          ...
          child: BackAppBar(label: readBuyMore?.title??"",
              isBack: true,
              isAction: false,
              onTap: (){
                Navigator.pop(context);
              ...
              child: Column(
                children: [
                  Row(
                    children: [
                      Container(
                        ...
                        child: Image.network(readBuyMore?.image??""),
                      ),
                      SizedBox(
                        width: 10,
                      ),
                  ...
                  GradientButton(
                      color: Color(0xffe6ecf0),
                      onPressed: () {
                        context.read<BuyMoreBloc>().add(GetBuyMoreReadEvent(readBuyMore?.id??0));
                        PersistentNavBarNavigator.pushNewScreen(
                          context,
                          screen:  BuyMoreGetForm(editDiscount: true,),
                  ...
                  GradientButton(
                      color: Color(0xffe6ecf0),
                      onPressed: () {
                        context.read<BuyMoreBloc>().add( DeleteBuyMoreEvent(readBuyMore?.id??0));
                      },
